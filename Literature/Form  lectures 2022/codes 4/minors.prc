...
*   The symbol xMinor is used to make sure we never get
...
*
Symbol xMinor(:1);
Local 'F' = <e_(1)*(`T'(1,1)+`minor'(1,1)*xMinor)>+...+
          <e_(`N')*(`T'(1,`N')+`minor'(1,`N')*xMinor)>;
#do k = 1,{`N'-1}
id  e_(i1?,...,i`k'?) = 
#do i = 1,`N'
    +e_(`i',i1,...,i`k')*('T'({`k'+1},`i')
...
if ( count(xMinor,1) == 0 ) Multiply `minor'(0,0);
id  xMinor= 1;
#endprocedure
