...
#define MAXCOMBI "10"
*	The optimum of MAXCOMBI seems to be 4 when chi is 14 or 12.
...
#do i = `n',1,-1
  ...
  #do ii = 1,1
	#$success = 0;
	#$pass = $pass+1;
	id,once,ifmatch->success,F(1)^3 = L`i'[l]^3+3*L`i'[l]^2*FF(1)+3*L`i'[l]*FF(1)^2+FF(1)^3;
	id,once,ifmatch->success,F(1)^2 = L`i'[l]^2+2*L`i'[l]*FF(1)+FF(1)^2;
	id,once,ifmatch->success,F(x?)^2 = L`i'[l^x]^2+2*L`i'[l^x]*FF(x)+FF(x)^2;
	id,once,ifnomatch->nomatch,F(x?) = L`i'[l^x]+FF(x);
	label success;
